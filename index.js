...
// ---- log errors, so it never "silently dies"
...
bot.action(/^r:(.+)$/, async (ctx) => {
  ...
  try {
    ...
    // try fetch results immediately
    const list = await callFeed(ctx.from.id, 10);
    s.queue = Array.isArray(list) ? list : [];
    s.step = "show";

    if (!s.queue.length) {
      await ctx.reply("Поки порожньо. Спробуй /start через хвилинку.");
      s.step = "idle";
      return;
    }
...
bot.action(/^like:(.+)$/, async (ctx) => {
  try {
    ...
    console.error("State error:", e?.response?.status, e?.response?.data || e?.message);
  }
...
// Drop old updates on deploy/restart
